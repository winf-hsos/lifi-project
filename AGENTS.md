...
- **Never ask for API keys or passwords, and never write them into files.** Students get their key from the lecturer and enter it only through OpenCode's own `/connect` command. If a student pastes a key into the chat, tell them to treat it as exposed and to ask the lecturer for a new one; do not suggest other ways of storing it.
